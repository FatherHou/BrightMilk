...
public class BrightFreshMilkSystem {
	...
	private CurrentOrder currentOrder1 = new CurrentOrder();
	/**
	...
	/**
	 * Add a product to the current order 鈥攊f the specified product is already
	...
	public void addCurrentOrderToSaleAndEmptyCurrentOrder() throws IOException {
		...
		// if(a==b){
		ArrayList<CurrentOrder> orderList = sale.getOrderList();
		currentOrder1=currentOrder;
		currentOrder=new CurrentOrder(new BigDecimal(0),new ArrayList<OrderItem>());
		orderList.add(currentOrder);
		sale.setOrderList(orderList);
//		for (CurrentOrder CurrentOrder : sale.getOrderList()) {
//			stdOut.println(CurrentOrder + "\n");
//		}
		System.out.println("Add successfully!");
		// }
//		Sale sale=new Sale(orderList);
		/* PLACE YOUR CODE HERE */

		// remove
		// stdErr.println(currentOrder.getOrderItemList().size());
		// else if(a==c){
//		int n = currentOrder.getOrderItemList().size();// getOrderItemList动态变化，需要固定变量
//		for (int i = 0; i < n; i++) {
//			// stdErr.println(i);
//			OrderItem orderItem = currentOrder.getOrderItemList().get(0);// get(0)从第一位删除防止因为LIST动态变化抛出异常
//			currentOrder.getOrderItemList().remove(0);
//			// subtract the money of the specified product
//			BigDecimal money = orderItem.getProduct().getPrice().multiply(new BigDecimal(orderItem.getQuantity()));
//			currentOrder.setOrderTotal(currentOrder.getOrderTotal().subtract(money));
//
...
//		System.out.println("Remove successfully!");
	...
	public void displaySale() throws IOException {
		...
		/* PLACE YOUR CODE HERE */
		int size = sale.getOrderList().size();
		// System.out.println(size);
		if (size == 0) {
			stdErr.println("The sale is empty");
		} else {
			System.out.println(currentOrder1.toString());
			}
//			stdErr.println(sale.toString());
		}
	}
